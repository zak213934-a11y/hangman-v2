create a new github compatible file for this code in the simplest way possible with minmal code and folders 

"""
Enhanced Pygame Hangman Game
Features:
- Graphical interface with pygame
- NLTK corpus for extensive word list
...
try:
    ...
    print("Then run: python -c 'import nltk; nltk.download(\"words\")'")
...
DIFFICULTY_SETTINGS = {
    ...
    "MEDIUM": {"min_length": 6, "max_length": 9, "max_attempts": 7, "hint_cost": 2},
    "HARD": {"min_length": 9, "max_length": 15, "max_attempts": 6, "hint_cost": 3},
}

# Default word list (fallback if NLTK unavailable)
DEFAULT_WORDS = [
    "python", "programming", "algorithm", "developer", "function", "variable",
    "computer", "keyboard", "software", "hardware", "internet", "database",
    "security", "network", "terminal", "compiler", "debugging", "framework",
    "repository", "authentication", "encryption", "bandwidth", "firewall",
]
...
class WordManager:
    ...
    def _initialize_words(self):
        ...
        # Ensure we have words for each difficulty
...
class HangmanGame:
    ...
    def masked_word(self) -> list[str]:
        ...
        return [
            letter if (letter in self.guessed_letters or i in self.revealed_hints) else "_"
    ...
    def get_hint(self) -> bool:
        ...
        if self.hints_used >= 3:  # Max 3 hints
            return False
    ...
    def is_won(self) -> bool:
        ...
        return all(
            letter in self.guessed_letters for letter in set(self.secret_word)
        )
...
class HangmanDrawing:
    ...
    def draw(self, screen: pygame.Surface, wrong_count: int):
        """Draw hangman based on wrong guess count."""
        # Gallows
        pygame.draw.line(screen, BLACK, (self.x, self.y + 200), (self.x + 100, self.y + 200), 5)  # Base
        pygame.draw.line(screen, BLACK, (self.x + 20, self.y + 200), (self.x + 20, self.y), 5)  # Pole
        pygame.draw.line(screen, BLACK, (self.x + 20, self.y), (self.x + 100, self.y), 5)  # Top
        pygame.draw.line(screen, BLACK, (self.x + 100, self.y), (self.x + 100, self.y + 30), 3)  # Rope
...
class HangmanGUI:
    ...
    def draw_menu(self):
        ...
        for i, (diff, color) in enumerate(zip(difficulties, colors)):
            ...
            if diff == self.difficulty:
                pygame.draw.rect(self.screen, ORANGE, btn.rect.inflate(10, 10), 4, border_radius=10)
    ...
    def draw_game(self):
        ...
        # Hint button
        hint_btn = Button(SCREEN_WIDTH - 150, SCREEN_HEIGHT - 70, 130, 50, f"HINT ({3 - self.game.hints_used})", ORANGE)
